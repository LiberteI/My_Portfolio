...
import { getRoomBounds } from "../../../config/sceneConfig"
...
export const buildRoomWallFigures = (scene, options = {}) => {
    const selectedConfigs = options.includeNames
        ? roomWallFigureConfigs.filter((config) => options.includeNames.includes(config.name))
        : roomWallFigureConfigs
